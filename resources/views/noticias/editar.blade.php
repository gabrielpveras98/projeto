...
@section('content')
    <div class="card card-register card-white" style="max-width: 500px; text-align: center; margin: auto;">
        <form class="form" method="post" action="/noticia/editar/{{ $noticia->id }}">
            @csrf
            <div class="card-body">
                <h1>Editando notícia {{ $noticia->titulo }}</h1>
                ...
                <div class="input-group{{ $errors->has('email') ? ' has-danger' : '' }}">
                    ...
                    <input type="text" name="subtitulo"
                           ...
                           placeholder="{{ _('subtítulo') }}" value="{{ $noticia->subtitulo }}">
                    {{--                    @include('alerts.feedback', ['field' => 'email'])--}}
                </div>
                <div class="input-group{{ $errors->has('password') ? ' has-danger' : '' }}">
                            <textarea type="text" name="conteudo"
                                      class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}"
                                      placeholder="conteúdo">{{ $noticia->conteudo }}</textarea>
                </div>
            </div>
            <div class="card-footer">
                <button type="submit" class="btn btn-primary btn-round btn-lg">{{ _('Confirmar alterações') }}</button>
            </div>
        </form>
    </div>
@endsection
